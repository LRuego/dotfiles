...
tailscale file get --loop --verbose "$TAILDROP_DIR" 2>&1 | while read -r line; do
    # Match the 'wrote filename as /path/to/file' pattern — supports spaces in filenames
    if [[ "$line" =~ wrote\ (.+)\ as\ (.+) ]]; then
        ...
        if [[ "$filename" =~ \.([jJ][pP][gG]|[jJ][pP][eE][gG]|[pP][nN][gG]|[gG][iI][fF]|[wW][eE][bB][pP]|[sS][vV][gG])$ ]]; then
            notify-send "TailDrop" "Received '$filename'" \
                -a "Tailscale" \
                -u normal \
